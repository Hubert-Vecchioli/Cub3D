...
void	ft_get_wall_direction_hit(t_game *game)
{
	...
	if (game->ray->side == 0 || game->ray->side == 3)
		game->ray->wall_x = game->player->posy + game->ray->perpwalldist
			* game->ray->raydiry;
	else
		game->ray->wall_x = game->player->posx + game->ray->perpwalldist
			* game->ray->raydirx;
...
void	ft_init_line(t_game *game)
{
	...
	game->line->off = 0;
	if (game->ray->x == WIDTH / 2 && game->ray->hit == 2
		&& game->ray->perpwalldist < 1)
		game->ray->aiming_at_door = 1;
	if (game->line->y >= HEIGHT - 1)
	{
		game->line->span = (game->ray->perpwalldist
				* game->tex_size);
		game->line->off = (game->tex_size * 0.5)
			* (1 - game->ray->perpwalldist);
	}
}
